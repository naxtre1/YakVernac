...
import { PowerTranslator, ProviderTypes, Translation, TranslatorConfiguration } from 'react-native-power-translator';
...
class Chat extends Component {
        ...
        firestore().collection('user').doc(''+receiverID).get().then(doc=>{
            if (doc.exists) {
                ...
                firestore().collection('chat').where('user', 'array-contains', this.props.user.uid).get().then(snapshot=>{
                    ...
                    this.messageRef.where(this.sender._id+'_state', '==', 'read').limit(100).get().then(snapshot=>{
                        const messages = []
                        for (const doc of snapshot.docs) {
                            const msg = doc.data()
                            const message = {
                        ...
                        this.setState(previousState => {
                            return {messages: GiftedChat.append(previousState.messages, messages), loading: false}
                        })
    ...
    _renderModalContent() {
       ...
       else if (this.state.freeTranslations > 10 && this.state.paid == false) {
            return (
    ...
    render() {
        TranslatorConfiguration.setConfig(ProviderTypes.Google, 'AIzaSyBcV8QCqM7YFYu6c9_6mjOt9bHDxo_F6sY', 'en');
        return (
