...
const ProfileScreen = (props) => {
    ...
    const [tempAllMedia, setTempAllMedia] = useState([])
    ...
    useEffect(() => {
        const lessonsMedia = allMedia.filter(item => {
            if (item.type == 'post' && item.isFromCreateGame != undefined && item.isFromCreateGame) {
                return true
            }
            return false
        })
        ...
        const picturesMedia = allMedia.filter(item => {
            if (item.type == 'post' && item.isFromCreateGame != undefined && !item.isFromCreateGame && item.content == 'image') {
                return true
            }
            return false
        })
        ...
        const videosMedia = allMedia.filter(item => {
            if (item.type == 'post' && item.isFromCreateGame != undefined && !item.isFromCreateGame && item.content == 'video') {
                return true
            }
            return false
        })
        ...
        const bookmarkAllMediaArray = allMedia.filter(item => {
            if (item.type == 'post' && item.bookmark) {
                return true
            }
            return false
        })
        ...
        //     if (item.type == 'post' || item.type == 'lesson' || item.type == 'video' || item.type == 'audio') {
        //         return true
        //     }
        //     return false
        // })
        // setTempAllMedia(data)
        const data = allMedia.filter(item => {
            if (item.type == 'post') {
                return true
            }
    ...
    const updateStatsUI = (progress) => {
        ...
        console.log("tempRating ------", tempRating['all']);
        ...
        console.log("errorQuarter : ", trueFalseErrorQuarter);

        //  M-2
        var uniqueWords = [];
        trueFalseErrorQuarter.forEach((item) => {
        ...
        // M-2
        var uniqueWords = [];
        trueFalseErrorYear.forEach((item) => {
            if (!uniqueWords.includes(item.word)) {
                uniqueWords.push(item.word);
            }
        });
    ...
    const changeNativeLang = lang => {
        ...
        firestore().collection('user').doc(user.uid).update({ languageLearning, languageNative }).then(() => {
            ...
            showMessage({
                message: `${strings('Profile.will_be_learning')} ${strings(`Profile.${languageLearning}`)} ${strings('Profile.now')}`,
                description: `${strings('Profile.redirecting')}`,
                type: "success",
            });
            // showMessage({
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
            <ScrollView style={{ backgroundColor: colors.mainBackground }}>
                <NavigationEvents
                    onDidFocus={payload => initPage(payload)}
                />
                <View style={{ marginHorizontal: 20, marginTop: 20 }}>
                    <View style={{ flexDirection: 'row' }}>
                        <View style={{ marginLeft: 20, justifyContent: 'center' }}>
                            <TouchableOpacity onPress={handleProfilePic}>
                                <Image
                                    defaultSource={require('../assets/profile-pic.png')}
                                    source={{ uri: myPic }} style={{ height: 95, width: 95, borderColor: colors.button, borderWidth: 1, borderRadius: 47.5, resizeMode: 'cover' }} />
                    ...
                    <View style={{ margin: 20 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <TouchableOpacity onPress={() => setEditDialog({ type: 'name', visible: true, title: strings('Profile.edit_name'), text: user.username })}>
                            ...
                            <Entypo name='edit' color={colors.blue} size={15} style={{ marginLeft: 5 }}
                                onPress={() => setEditDialog({ type: 'name', visible: true, title: strings('Profile.edit_name'), text: user.username })} />
                        </View>
                        {
                            user.introduction ?
                                <TouchableOpacity onPress={() => setEditDialog({ type: 'introduction', visible: true, title: strings('Profile.edit_introduction'), text: user.introduction })}>
                                    <Text style={{ marginVertical: 15, fontSize: 14, color: colors.text }}>{user.introduction}
                                        ...
                                        <Entypo name='edit' color={colors.blue} size={15} style={{ marginLeft: 10 }}
                                            onPress={() => setEditDialog({ type: 'introduction', visible: true, title: strings('Profile.edit_introduction'), text: user.introduction })} />
                                    </Text></TouchableOpacity> :
                                <Text style={{ marginVertical: 15, fontSize: 14, color: colors.text, fontStyle: 'italic' }}>
                                    {strings('Profile.add_introduction')}
                                    <Text> </Text>
                                    <Entypo name='edit' color={colors.blue} size={15} style={{ marginLeft: 10 }}
                                        onPress={() => setEditDialog({ type: 'introduction', visible: true, title: strings('Profile.edit_introduction'), text: '' })} />
                                </Text>
                        }
                        <SwitchSelector
                            style={{ width: 80 }}
                    ...
                    <TouchableOpacity
                        ...
                        <Image resizeMode='contain' source={languageNative == 'EN' ? require('../assets/boost.png') : require('../assets/boost_pt.png')} style={{ alignSelf: 'center' }} />
                    </TouchableOpacity>
                    ...
                    </View>
                    {
                        ...
                        <View style={{ marginHorizontal: 20, flexDirection: 'row', justifyContent: 'space-between' }}>
                            <TouchableOpacity
                                ...
                                <Text style={{ width: 64, height: 20, backgroundColor: 'white', opacity: photoButton == 'all' ? 1 : 0.5, color: 'grey', textAlign: 'center', fontSize: 12, textAlignVertical: 'center' }}>{strings('Profile.all')}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                onPress={() => setPhotoButton('lessons')}>
                                <Text style={{ width: 64, height: 20, backgroundColor: 'white', opacity: photoButton == 'lessons' ? 1 : 0.5, color: 'grey', textAlign: 'center', fontSize: 12, textAlignVertical: 'center' }}>{strings('Profile.lessons')}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                onPress={() => setPhotoButton('pictures')}>
                                <Text style={{ width: 64, height: 20, backgroundColor: 'white', opacity: photoButton == 'pictures' ? 1 : 0.5, color: 'grey', textAlign: 'center', fontSize: 12, textAlignVertical: 'center' }}>{strings('Profile.pictures')}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                onPress={() => setPhotoButton('videos')}>
                                <Text style={{ width: 64, height: 20, backgroundColor: 'white', opacity: photoButton == 'videos' ? 1 : 0.5, color: 'grey', textAlign: 'center', fontSize: 12, textAlignVertical: 'center' }}>{strings('Profile.videos')}</Text>
                            </TouchableOpacity>
                        </View>
                    ...
                    }
                    {
                        ...
                        <View style={{ marginHorizontal: 20, flexDirection: 'row', justifyContent: 'space-between' }}>
                            <TouchableOpacity
                                ...
                                <Text style={{ width: 64, height: 20, backgroundColor: 'white', opacity: lessonButton == 'all' ? 1 : 0.5, color: 'grey', textAlign: 'center', fontSize: 12, textAlignVertical: 'center' }}>{strings('Profile.all')}</Text>
                            </TouchableOpacity>
                            {/* <TouchableOpacity
                    ...
                    }
                    {
                        ...
                        <View style={{ marginHorizontal: 20, flexDirection: 'row', justifyContent: 'space-between' }}>
                            <TouchableOpacity
                                ...
                                <Text style={{ width: 64, height: 20, backgroundColor: 'white', opacity: bookmarkButton == 'all' ? 1 : 0.5, color: 'grey', textAlign: 'center', fontSize: 12, textAlignVertical: 'center' }}>{strings('Profile.all')}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                onPress={() => setBookmarkButton('lessons')}>
                                <Text style={{ width: 64, height: 20, backgroundColor: 'white', opacity: bookmarkButton == 'lessons' ? 1 : 0.5, color: 'grey', textAlign: 'center', fontSize: 12, textAlignVertical: 'center' }}>{strings('Profile.lessons')}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                onPress={() => setBookmarkButton('pictures')}>
                                <Text style={{ width: 64, height: 20, backgroundColor: 'white', opacity: bookmarkButton == 'pictures' ? 1 : 0.5, color: 'grey', textAlign: 'center', fontSize: 12, textAlignVertical: 'center' }}>{strings('Profile.pictures')}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                onPress={() => setBookmarkButton('videos')}>
                                <Text style={{ width: 64, height: 20, backgroundColor: 'white', opacity: bookmarkButton == 'videos' ? 1 : 0.5, color: 'grey', textAlign: 'center', fontSize: 12, textAlignVertical: 'center' }}>{strings('Profile.videos')}</Text>
                            </TouchableOpacity>
                        </View>
                    }
                </View>
                        ...
                        <View style={{ margin: 20, paddingLeft: 30, paddingRight: 20 }}>
                            <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
                                <TouchableOpacity
                                    ...
                                    <Text style={{ fontSize: 12, color: lessonDuration == 'quarter' ? 'black' : colors.text }}>{strings('Profile.per_quarter')}</Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    onPress={() => {
                                    ...
                                    <Text style={{ fontSize: 12, color: lessonDuration == 'year' ? 'black' : colors.text, marginLeft: 20 }}>{strings('Profile.year')}</Text>
                                </TouchableOpacity>
                            </View>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                ...
                                lessonButton == languageLearning ?
                                    <View style={{ flexDirection: 'row', margin: 20 }}>
                                        <Progress.Circle
                                            ...
                                            formatText={progress => ((vocabularyCnt[languageLearning] * 100) / 500) + '%'}
                                        // formatText={progress => ((vocabularyCntQuarter['all'] * 100) / 500)+ '%'}
                                        // formatText={progress => Math.round(rating[languageLearning] * 100) + '%'}
                                        />
                                        {
                                            successWordCnt[languageLearning] < 500 &&
                                            <View style={{ marginLeft: 20 }}>
                                                <Text style={[{ ...styles.text2 }, { fontWeight: 'bold' }]}>{strings('Profile.beginner')}</Text>
                                                <Text style={styles.text2}>0 - 500 word</Text>
                                                <Text style={styles.text2}>{strings('Profile.vocabulary')}</Text>
                                            </View>
                                        ...
                                        }
                                        {
                                            ...
                                            <View style={{ marginLeft: 20 }}>
                                                <Text style={[{ ...styles.text2 }, { fontWeight: 'bold' }]}>{strings('Profile.intermediate')}</Text>
                                        ...
                                        }
                                        {
                                            ...
                                            <View style={{ marginLeft: 20 }}>
                                                <Text style={[{ ...styles.text2 }, { fontWeight: 'bold' }]}>{strings('Profile.advanced')}</Text>
                                        ...
                                        }
                                        {
                                            ...
                                            <View style={{ marginLeft: 20 }}>
                                                <Text style={[{ ...styles.text2 }, { fontWeight: 'bold' }]}>{strings('Profile.fluent')}</Text>
                            ...
                            <Text style={styles.subtitle}>{strings('Profile.learning_styles')}</Text>
                            <View style={styles.lessonLine}>
                                <Text style={styles.text1}>{strings('Profile.listen')}</Text>
                                <View style={{ justifyContent: 'center', flex: 1, height: 35 }}>
                                    <Text style={{ position: 'absolute', fontSize: 9, color: colors.blue, top: 0, left: (listenRating[lessonButton] * 100 - 8) + '%' }}>{(listenRating[lessonButton] * 100).toFixed(2)}%</Text>
                                    <Progress.Bar progress={listenRating[lessonButton]} width={null} height={10} />
                                </View>
                            </View>
                            <View style={styles.lessonLine}>
                                <Text style={styles.text1}>{strings('Profile.speak')}</Text>
                                <View style={{ justifyContent: 'center', flex: 1, height: 35 }}>
                                    <Text style={{ position: 'absolute', fontSize: 9, color: colors.blue, top: 0, left: (speakRating[lessonButton] * 100 - 8) + '%' }}>{(speakRating[lessonButton] * 100).toFixed(2)}%</Text>
                                    <Progress.Bar progress={speakRating[lessonButton]} width={null} height={10} />
                                </View>
                            </View>
                            <View style={styles.lessonLine}>
                                <Text style={styles.text1}>{strings('Profile.write')}</Text>
                                <View style={{ justifyContent: 'center', flex: 1, height: 35 }}>
                                    <Text style={{ position: 'absolute', fontSize: 9, color: colors.blue, top: 0, left: (writeRating[lessonButton] * 100 - 8) + '%' }}>{(writeRating[lessonButton] * 100).toFixed(2)}%</Text>
                                    <Progress.Bar progress={writeRating[lessonButton]} width={null} height={10} />
                                </View>
                            </View>
                            <View style={styles.lessonLine}>
                                <Text style={styles.text1}>{strings('Profile.see')}</Text>
                                <View style={{ justifyContent: 'center', flex: 1, height: 35 }}>
                                    <Text style={{ position: 'absolute', fontSize: 9, color: colors.blue, top: 0, left: (seeRating[lessonButton] * 100 - 8) + '%' }}>{(seeRating[lessonButton] * 100).toFixed(2)}%</Text>
                                    <Progress.Bar progress={seeRating[lessonButton]} width={null} height={10} />
                                </View>
                            </View>
                        </View>
                        <View style={{ marginLeft: 20, marginRight: 20 }}>
                            <Text style={styles.subtitle}>{strings('Profile.word_stats')}</Text>
                            <LinearGradient
                                start={{ x: 0, y: 0 }}
                                end={{ x: 1, y: 0 }}
                                ...
                                <Text style={styles.text3}>{strings('Profile.vocabulary')}</Text>
                                <Text style={styles.text4}>{vocabularyCnt[lessonButton]}</Text>
                                <Text style={styles.text3}>{strings('Profile.completed')}</Text>
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                                <LinearGradient
                                    ...
                                    <Text style={styles.text3}>{strings('Profile.attempt')}</Text>
                                    <Text style={styles.text4}>{successWordCnt[lessonButton]}</Text>
                                    <Text style={styles.text3}>{strings('Profile.success')}</Text>
                                ...
                                <LinearGradient
                                    ...
                                    <Text style={styles.text3}>{strings('Profile.non_success')}</Text>
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '100%', marginBottom: 20 }}>
                                <LinearGradient
                                    ...
                                    <Text style={styles.text3}>{strings('Profile.lessons')}</Text>
                                ...
                                <LinearGradient
                                    ...
                                    <Text style={styles.text3}>{strings('Profile.translations')}</Text>
                                    <Text style={styles.text4}>{translationCnt[lessonButton]}</Text>
                                    <Text style={styles.text3}>{strings('Profile.used')}</Text>
                        ...
                        </View>
                        {
                            lessonButton == languageLearning ?
                                <>
                                    <View style={{ flexDirection: 'row' }}>
                                        <View style={{ flex: 1 }}>
                                            <Text style={styles.subtitle}>{strings('Profile.time_spent')}</Text>
                                            <Text style={[{ ...styles.text2 }, { textAlign: 'center' }]}>
                                                {durationString}
                                            </Text>
                                        </View>
                                        <View style={{ flex: 1 }}>
                                            <Text style={styles.subtitle}>{strings('Profile.study_buddies')}</Text>
                                            <Text style={[{ ...styles.text2 }, { textAlign: 'center' }]}>{buddies}</Text>
                                        </View>
                                    </View>
                                    <Text style={styles.subtitle}>{strings('Profile.priority_list')}</Text>
                                    <View style={{ marginLeft: 50, marginBottom: 20, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', width: '100%' }}>
                                        <View>
                                            <View
                                                ...
                                                <LinearGradient
                                                    ...
                                                    <Text style={styles.text3}>{strings('Profile.hearing')}</Text>
                                            ...
                                            <View
                                                ...
                                                <LinearGradient
                                                    ...
                                                    colors={['#3B96FD', '#449DFD', '#4AA1FD']}
                                                    style={styles.gradient2}>
                                                    <Text style={styles.text3}>{strings('Profile.speaking')}</Text>
                                                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                                                        <Text style={[{ ...styles.text4 }, { flex: 1, textAlign: 'center' }]}>
                                            ...
                                            <View
                                                ...
                                                <LinearGradient
                                                    ...
                                                    colors={['#3B96FD', '#449DFD', '#4AA1FD']}
                                                    style={styles.gradient2}>
                                                    <Text style={styles.text3}>{strings('Profile.writing')}</Text>
                                                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                                                        <Text style={[{ ...styles.text4 }, { flex: 1, textAlign: 'center' }]}>
                                            ...
                                            <View
                                                ...
                                                <LinearGradient
                                                    ...
                                                    colors={['#3B96FD', '#449DFD', '#4AA1FD']}
                                                    style={styles.gradient2}>
                                                    <Text style={styles.text3}>{strings('Profile.sight')}</Text>
                                                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                                                        <Text style={[{ ...styles.text4 }, { flex: 1, textAlign: 'center' }]}>
                                        ...
                                        <View style={{ marginLeft: 20, flex: 1 }}>
                                            <Text style={[{ ...styles.text2Bold }, { marginTop: 20, marginBottom: 20 }]}>{strings('Profile.top_errors')}</Text>
                                            <View style={{ flex: 1 }}>
                                                {
                ...
                }
                {
                    ...
                    <>
                        <View style={{ backgroundColor: 'white', padding: 20 }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Text style={{ color: colors.text }}>{strings('Profile.native_lang')}</Text>
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 20 }}>
                                <Text style={{ color: colors.text }}>{strings('Profile.notifications')}</Text>
                                <SwitchSelector
                                    style={{ width: 70 }}
                                    initial={user.notify}
                                    height={30}
                        ...
                        <Text style={styles.text5}>{strings('Profile.notice')}</Text>
                        <TouchableOpacity
                            style={{ backgroundColor: '#D83138', borderRadius: 10, marginTop: 5, padding: 15, marginHorizontal: 30 }}
                            onPress={() => setRemoveModalVisible(true)}
                        >
                            <Text style={{ fontSize: 14, color: 'white', textAlign: 'center' }}>{strings('Profile.delete_account')}</Text>
                        </TouchableOpacity>
                ...
                }
                {
                    ...
                    <>
                        ...
                        {photoButton == 'all' && <ImageGrid all={true} setLoading={setLoading} refreshPage={refreshPage} segment={segment} media={tempAllMedia} callbackBookmark={handleBookmark} />}
                        {photoButton == 'lessons' && <ImageGrid all={true} setLoading={setLoading} refreshPage={refreshPage} segment={segment} media={lessonsMedia} callbackBookmark={handleBookmark} />}
                        {photoButton == 'pictures' && <ImageGrid all={true} setLoading={setLoading} refreshPage={refreshPage} segment={segment} media={picturesMedia} callbackBookmark={handleBookmark} />}
                        {photoButton == 'videos' && <ImageGrid all={true} setLoading={setLoading} segment={segment} refreshPage={refreshPage} media={videosMedia} callbackBookmark={handleBookmark} />}
                    </>
                }
                {
                    segment == 'bookmark' &&
                    <>
                        {bookmarkButton == 'all' && <ImageGrid all={false} setLoading={setLoading} refreshPage={refreshPage} segment={segment} media={bookmarkAllMedia} callbackBookmark={handleBookmark} />}
                        {bookmarkButton == 'lessons' && <ImageGrid all={false} setLoading={setLoading} refreshPage={refreshPage} segment={segment} media={bookmarkLessonsMedia} callbackBookmark={handleBookmark} />}
                        {bookmarkButton == 'pictures' && <ImageGrid all={false} setLoading={setLoading} refreshPage={refreshPage} segment={segment} media={bookmarkPicturesMedia} callbackBookmark={handleBookmark} />}
                        {bookmarkButton == 'videos' && <ImageGrid all={false} setLoading={setLoading} refreshPage={refreshPage} segment={segment} media={bookmarkVideosMedia} callbackBookmark={handleBookmark} />}
                    </>
                }
            </ScrollView>
            ...
            >
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginTop: 10 }}>
                    <TouchableOpacity
                        ...
                        <Text style={{ color: 'white' }}>{strings('Profile.save')}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        ...
                        <Text style={{ color: colors.blue }}>{strings('Profile.cancel')}</Text>
                    </TouchableOpacity>
